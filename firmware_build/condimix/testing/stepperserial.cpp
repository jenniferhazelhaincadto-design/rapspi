...
bool delayWithSafety(unsigned long durationMs) {
...
void applyDryDispense(JsonArrayConst dry) {
  ...
  if (currentCatcherPos > 0 && !stopRequested) {
    ...
    if (!stepWithEmergencyStop(&stepper7, STEPS_PER_REV * currentCatcherPos)) {
      return;
    }
    delayWithSafety(3000);
  }
}
...
void handleClean() {
  ...
  // Trigger all relays to flush pipes for a few seconds
  Serial.println("Flushing wet conduits...");
  for (size_t i = 0; i < RELAY_COUNT; i++) {
    digitalWrite(RELAY_PINS[i], RELAY_ACTIVE_LEVEL);
  }
  delayWithSafety(5000);
  allRelaysOff();
